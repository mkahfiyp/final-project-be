...
import { UserSkillAddDataDTO, UserSkillGetAllDataByUserIdDTO } from "../dto/userSkill.dto"

class UserSkillRepository {
    getAllUserSkillByUserId = async (user_id: number) => {
        const data: UserSkillGetAllDataByUserIdDTO[] = await prisma.userSkills.findMany({
            where: {
                userId: user_id,
            }, include: {
                skill: true,
