...
class InterviewService {
   ...
   */
  async getInterviews(userId: number, userRole: string, page: number = 1, limit: number = 10) {
    try {
      if (userRole === 'USER') {
        // Get interviews where user is the candidate
        const result = await this.interviewRepository.getInterviewsForUser(userId, page, limit);
        return {
          data: result.data,
          pagination: {
            page: result.page,
        ...
        const result = await this.interviewRepository.getInterviewsForCompany(company.company_id, page, limit);
        return {
          data: result.data,
          pagination: {
            page: result.page,
   ...
   */
  async scheduleInterview(userId: number, applicationId: number, schedule: string) {
    // Validate required fields
    if (!applicationId || !schedule) {
      throw new AppError("Missing required fields: application_id, schedule", 400);
    }
    ...
    try {
      ...
      if (application.Jobs?.company_id !== company.company_id) {
        throw new AppError("Unauthorized to schedule interview for this application", 403);
      }
      ...
      return await this.interviewRepository.createInterview(applicationId, scheduleDate);
    } catch (error) {
      if (error instanceof AppError) {
        throw error;
      }
   ...
   */
  async updateInterviewSchedule(interviewId: number, schedule: string, userId: number) {
    // Validate schedule date
    const scheduleDate = new Date(schedule);
    if (isNaN(scheduleDate.getTime())) {
      throw new AppError("Invalid schedule date format", 400);
    ...
    try {
      ...
      if (!company || interview.application.Jobs?.company_id !== company.company_id) {
        throw new AppError("Unauthorized to update this interview", 403);
      }

      return await this.interviewRepository.updateInterviewSchedule(interviewId, scheduleDate);
    } catch (error) {
      if (error instanceof AppError) {
        throw error;
      }
  ...
  async getInterviewDetail(interviewId: number, userId: number) {
    try {
      ...
      if (!isCandidate) {
        ...
        const isCompanyOwner = company && interview.application.Jobs?.company_id === company.company_id;
